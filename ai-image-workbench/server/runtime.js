...
export class WorkbenchRuntime {
  ...
  async generateImage(task, index) {
    ...
    if (task.provider === 'gpt_image_2_5' && process.env.GPT_IMAGE_2_5_API_KEY) return this.generateWithImage2(task, outputId, outputPath);
  ...
  async generateWithImage2(task, outputId, outputPath) {
    const isGptImage25 = task.provider === 'gpt_image_2_5'; const apiKey = isGptImage25 ? process.env.GPT_IMAGE_2_5_API_KEY : process.env.OPENAI_API_KEY; const configuredBase = ((isGptImage25 ? process.env.GPT_IMAGE_2_5_BASE_URL : process.env.LLM_BASE_URL) || 'https://api.openai.com/v1').replace(/\/$/, ''); const base = configuredBase.replace(/\/images\/(?:edits|generations)$/i, ''); const refs = await Promise.all((task.referenceAssetIds || []).map((assetId) => this.asset(assetId))); const mode = refs.length ? 'image-to-image' : 'text-to-image'; const endpoint = process.env.OPENAI_IMAGE_GENERATION_URL || (refs.length ? process.env.OPENAI_IMAGE_EDIT_URL : null) || `${base}/images/${refs.length ? 'edits' : 'generations'}`; const settings = task.settings || {}; const requestMeta = { phase: 'provider.request', requestId: task.requestId, taskId: task.taskId, provider: task.provider, mode, method: 'POST', url: endpoint, model: task.model || this.modelFor(task.provider), headers: { authorizationPresent: Boolean(apiKey), contentType: refs.length ? 'multipart/form-data' : 'application/json' }, body: { prompt: task.prompt, size: this.gptSizeFor(settings), quality: settings.quality || 'auto', background: settings.background || 'auto', moderation: 'low', referenceCount: refs.length } }; await this.debugLog(requestMeta);
    try { let response; const timeout = AbortSignal.timeout(Number(process.env.PROVIDER_TIMEOUT_MS || 180000)); const settings = task.settings || {}; const size = this.gptSizeFor(settings); if (refs.length) { const form = new FormData(); form.set('model', requestMeta.model); form.set('prompt', task.prompt); form.set('size', size); form.set('quality', settings.quality || 'auto'); form.set('background', settings.background || 'auto'); form.set('moderation', 'low'); for (const ref of refs) form.append('image[]', new Blob([await fs.readFile(ref.localPath)], { type: ref.mimeType || 'image/png' }), path.basename(ref.localPath)); response = await fetch(endpoint, { method: 'POST', headers: { Authorization: `Bearer ${apiKey}` }, body: form, signal: timeout }); } else response = await fetch(endpoint, { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: requestMeta.model, prompt: task.prompt, size, quality: settings.quality || 'auto', background: settings.background || 'auto', moderation: 'low' }), signal: timeout }); const raw = await response.text(); let data; try { data = raw ? JSON.parse(raw) : {}; } catch { data = { raw }; } await this.debugLog({ phase: 'provider.response', requestId: task.requestId, taskId: task.taskId, provider: task.provider, status: response.status, ok: response.ok, body: typeof raw === 'string' ? raw.slice(0, 4000) : raw }); if (!response.ok) throw new Error(data?.error?.message || `image2 API ${response.status}`); await this.writeProviderImage(data, outputPath); return { assetId: outputId, url: `/api/assets/${outputId}`, localPath: outputPath, mimeType: 'image/png', providerDetails: { mode, endpoint } }; } catch (error) { await this.debugLog({ phase: 'provider.error', requestId: task.requestId, taskId: task.taskId, provider: task.provider, error: { name: error.name, message: error.message, cause: error.cause ? { name: error.cause.name, message: error.cause.message, code: error.cause.code, errno: error.cause.errno, syscall: error.cause.syscall } : null, stack: error.stack } }); throw error; }
  }
  async generateWithNanoBanana(task, outputId, outputPath) {
    const configuredBase = (process.env.OPENLUX_BASE_URL || 'https://api.openlux.ai/v1').replace(/\/$/, ''); const base = configuredBase.replace(/\/v1$/, ''); const model = process.env.OPENLUX_IMAGE_MODEL || process.env.GOOGLE_IMAGE_MODEL || 'gemini-2.5-flash-image'; const settings = task.settings || {}; const imageConfig = { imageSize: settings.resolutionTier || '1K' }; if ((settings.aspectRatio || task.ratio || 'auto') !== 'auto') imageConfig.aspectRatio = settings.aspectRatio || task.ratio; const refs = await Promise.all((task.referenceAssetIds || []).map((assetId) => this.asset(assetId))); const referenceParts = await Promise.all(refs.map(async (asset) => ({ inlineData: { mimeType: asset.mimeType || 'image/png', data: (await fs.readFile(asset.localPath)).toString('base64') } }))); const endpoint = `${base}/v1beta/models/${encodeURIComponent(model)}:generateContent`; const body = { contents: [{ role: 'user', parts: [{ text: task.prompt }, ...referenceParts] }], generationConfig: { responseModalities: ['IMAGE'], imageConfig } }; await this.debugLog({ phase: 'provider.request', requestId: task.requestId, taskId: task.taskId, provider: 'nano_banana', method: 'POST', url: endpoint, model, body: { imageConfig, referenceCount: refs.length } }); let response; let data; for (let attempt = 0; attempt < 3; attempt += 1) { response = await fetch(endpoint, { method: 'POST', headers: { Authorization: `Bearer ${process.env.OPENLUX_API_KEY || process.env.GOOGLE_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: AbortSignal.timeout(Number(process.env.PROVIDER_TIMEOUT_MS || 180000)) }); data = await response.json(); const message = data?.error?.message || ''; if (response.ok || !/saturat|overload|temporar|rate limit/i.test(message)) break; await new Promise((resolve) => setTimeout(resolve, 1500 * (attempt + 1))); } await this.debugLog({ phase: 'provider.response', requestId: task.requestId, taskId: task.taskId, provider: 'nano_banana', status: response.status, ok: response.ok, body: { hasImage: Boolean(data?.candidates?.[0]?.content?.parts?.some((part) => part.inlineData?.data)) } }); if (!response.ok) throw new Error(data?.error?.message || `Nano Banana API ${response.status}`); await this.writeNanoBananaImage(data, outputPath); return { assetId: outputId, url: `/api/assets/${outputId}`, localPath: outputPath, mimeType: 'image/png', providerDetails: { mode: task.type, endpoint, imageConfig, attempts: 1 } };
  }
  ...
  modelFor(provider) { if (provider === 'nano_banana') return process.env.OPENLUX_IMAGE_MODEL || process.env.GOOGLE_IMAGE_MODEL || 'gemini-2.5-flash-image'; if (provider === 'gpt_image_2_5') return process.env.GPT_IMAGE_2_5_MODEL || 'gpt-image-2.5-sunburst'; return process.env.OPENAI_IMAGE_MODEL || 'gpt-image-2'; }
